...
public class Cache {
	...
	public static void loadFromDatabase() throws Exception {
		...
		/*
		 * _connection.setAutoCommit( false ); CallableStatement sqlProc;
		 * sqlProc = _connection.prepareCall("{?=call getAllSessions( )}");
		 * sqlProc.registerOutParameter( 1, Types.OTHER ); sqlProc.execute( );
		 * ResultSet resultSet = (ResultSet) sqlProc.getObject( 1 ); while(
		 * resultSet.next( ) ) { strValue= Integer.toString( resultSet.getInt( 2
		 * ) ); strKey = resultSet.getString( 3 ); System.out.println( strKey +
		 * " .... " + strValue ); _htblCache.put( strKey , strValue ); }
		 * resultSet.close( );
		 *
		 * sqlProc.close( );
		 */
	...
	protected static void connectToDB(String strAddress, int nPort, String strDBName,
